...
export default function RoleReveal({
  ...
  if (!me.role) {
    return <p className="text-center text-sm text-neutral-500 p-6">역할을 배정하는 중...</p>;
  }
    ...
    <div className="w-full max-w-md mx-auto space-y-6 p-6">
      {!revealed ? (
        <button
          ...
          className="w-full aspect-[3/4] rounded-xl border-2 border-dashed border-neutral-300 dark:border-neutral-700 flex items-center justify-center text-lg font-medium"
        >
          탭하여 내 역할 확인하기
          <br />
        ...
        >
          <div className="text-center space-y-1">
            <p className="text-xs uppercase tracking-widest opacity-80">{evil ? "악당 진영" : "선 진영"}</p>
            <p className="text-3xl font-bold">{ROLE_LABEL[role]}</p>
          </div>
          <p className="text-sm opacity-90 text-center">{ROLE_DESCRIPTION[role]}</p>
          ...
          {visibleInfo.map((info) => (
            <div key={info.label} className="rounded-lg bg-black/20 p-3 space-y-1">
              ...
              {info.players.length === 0 ? (
                <p className="text-sm italic opacity-70">정보 없음</p>
              ) : (
          ...
          <button
            ...
            className="w-full rounded-md bg-white/10 py-2 text-sm font-medium"
          >
            숨기기
          </button>
        </div>
      ...
      {me.is_host && (
        <button
          ...
          className="w-full rounded-md bg-neutral-900 text-white dark:bg-white dark:text-neutral-900 py-2.5 font-medium"
        >
          모두 확인했어요, 게임 시작하기
        </button>
      )}
