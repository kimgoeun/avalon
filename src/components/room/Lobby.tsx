  ...
  return (
    <div className="w-full max-w-md mx-auto space-y-6 p-6">
      <div className="text-center space-y-1">
        ...
        <p className="text-xs text-neutral-500">이 코드를 다른 사람에게 공유해서 초대하세요</p>
      </div>

      <div className="space-y-2">
        <p className="text-sm font-medium">참가자 ({count}/{MAX_PLAYERS})</p>
        {isHost && (
          ...
          {players.map((p, i) => (
            ...
            >
              <span className="flex items-center gap-2">
                ...
                {p.is_host && <span className="text-xs text-amber-500">방장</span>}
              </span>
              {isHost && (
                <span className="flex items-center gap-1">
                  <button
                    ...
                    className="w-6 h-6 flex items-center justify-center rounded border border-neutral-300 dark:border-neutral-700 disabled:opacity-30"
                  >
                    ▲
                  </button>
                  <button
        ...
        {count < MIN_PLAYERS && (
          <p className="text-xs text-neutral-500">최소 {MIN_PLAYERS}명이 필요합니다 ({MIN_PLAYERS - count}명 더 필요)</p>
        )}
      </div>
      ...
      {isHost ? (
        <div className="space-y-4">
          <div className="space-y-2">
            <p className="text-sm font-medium">역할 옵션</p>
        ...
        <p className="text-center text-sm text-neutral-500">방장이 게임을 시작할 때까지 기다려주세요...</p>
      )}
    </div>
  );
}
...
function RoleToggle({
  ...
  return (
    <label
      className={`flex items-center gap-2 rounded-md border border-neutral-200 dark:border-neutral-800 px-3 py-2 ${
