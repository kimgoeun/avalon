...
import { createRoom, joinRoom } from "@/lib/sevenpoker-actions";
import { saveSession } from "@/lib/session";

export default function SevenPokerHomePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === "create") {
        ...
        router.push(`/sevenpoker/room/${room.code}`);
      } else {
        if (!code.trim()) {
    ...
    <main className="flex flex-1 items-center justify-center p-6">
      ...
      <div className="w-full max-w-sm space-y-6">
        <div className="text-center space-y-1">
          <h1 className="text-4xl font-bold tracking-tight">세븐포커 칩 배팅</h1>
          <p className="text-base text-neutral-500">카드는 오프라인으로, 칩은 여기서 관리해요</p>
        </div>
        ...
        <p className="text-center text-sm text-neutral-500">2~7명이 함께 플레이할 수 있어요</p>
      </div>
    </main>
  );
}
