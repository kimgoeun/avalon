...
import { useSevenPokerRoom } from "@/hooks/useSevenPokerRoom";
import { removePlayerFromRoom } from "@/lib/sevenpoker-actions";
...
export default function RoomClient({ roomCode }: { roomCode: string }) {
  ...
  const { room, players, loading, error, refetch } = useSevenPokerRoom(roomCode);
  const session = typeof window !== "undefined" ? loadSession(roomCode, "sevenpoker") : null;
  ...
  async function leaveRoom() {
    ...
    clearSession(roomCode, "sevenpoker");
    router.push("/sevenpoker");
  }
  ...
  switch (room.phase) {
    case "lobby":
      ...
      break;
    case "select_first_actor":
    case "betting":
    case "select_winner":
      content = <Table room={room} players={players} me={me} />;
      break;
    case "game_over":
