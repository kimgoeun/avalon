...
import { createRoom, joinRoom } from "@/lib/liar-game-actions";
import { saveSession } from "@/lib/session";

export default function LiarGameHomePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === "create") {
        ...
        saveSession({ playerId: player.id, roomId: room.id, roomCode: room.code }, "liar-game");
        router.push(`/liar-game/room/${room.code}`);
      } else {
        if (!code.trim()) {
  ...
  return (
    <main className="flex flex-1 items-center justify-center p-6">
      <Link href="/" className="fixed top-4 left-4 text-sm text-neutral-500 hover:underline">
        ← 게임 목록
      </Link>
      <div className="w-full max-w-sm space-y-6">
        <div className="text-center space-y-1">
          <h1 className="text-4xl font-bold tracking-tight">라이어 게임</h1>
          <p className="text-base text-neutral-500">누가 제시어를 모르는 라이어일까요?</p>
        </div>
        ...
        <p className="text-center text-sm text-neutral-500">3~20명이 함께 플레이할 수 있어요</p>
      </div>
    </main>
  );
}
