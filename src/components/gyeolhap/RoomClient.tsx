...
import { useGyeolhapRoom } from "@/hooks/useGyeolhapRoom";
import { removePlayerFromRoom } from "@/lib/gyeolhap-actions";
...
export default function RoomClient({ roomCode }: { roomCode: string }) {
  ...
  const { room, players, boardCards, loading, error, refetch } = useGyeolhapRoom(roomCode);
  const session = typeof window !== "undefined" ? loadSession(roomCode, "gyeolhap") : null;
  ...
  async function leaveRoom() {
    ...
    clearSession(roomCode, "gyeolhap");
    router.push("/gyeolhap");
  }
  ...
  switch (room.phase) {
    case "lobby":
      ...
      break;
    case "playing":
      content = <Board room={room} players={players} boardCards={boardCards} me={me} />;
      break;
    case "game_over":
